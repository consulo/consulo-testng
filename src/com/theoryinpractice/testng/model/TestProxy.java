...
{
	@NonNls
	public static final Pattern COMPARISION_PATTERN = Pattern.compile("(.*)expected same with:\\<(.*)\\> but was:\\<(.*)\\>.*", Pattern.DOTALL);
	@NonNls
	public static final Pattern EXPECTED_BUT_WAS_PATTERN = Pattern.compile("(.*)expected:\\<(.*)\\> but was:\\<(.*)\\>.*", Pattern.DOTALL);
	@NonNls
	...
	public boolean isNotPassed()
	{
		...
		if(resultMessage == null && results.size() == 0)
		{
			return true;
		}
		for(TestProxy child : results)
		{
			if(child.isNotPassed())
			{
				return true;
			}
		}
	...
	{
		if(result.getResult() == MessageHelper.PASSED_TEST && Registry.is("testng.skip.expected.exceptions"))
		{
			return;
		}
