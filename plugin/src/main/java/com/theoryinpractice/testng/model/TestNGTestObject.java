...
public abstract class TestNGTestObject
{
	...
	public static final String[] GROUPS_CONFIGURATION = {
	...
	protected static void calculateDependencies(PsiMethod[] methods, final Map<PsiClass, Map<PsiMethod, List<String>>> results, GlobalSearchScope searchScope, @Nullable final PsiClass... classes)
	{
			...
			final PsiClass... classes)
	{
		ApplicationManager.getApplication().runReadAction(() ->
		{
			...
			for(PsiMember psiMember : AnnotatedMembersSearch.search(testAnnotation, searchScope))
			{
				...
				if(skipUnrelated && ArrayUtil.find(classes, containingClass) < 0)
				{
					continue;
				}
			...
			Map<PsiClass, Map<PsiMethod, List<String>>> results)
	{
		...
		{
			if(AnnotationUtil.isAnnotated(method, TestNGUtil.TEST_ANNOTATION_FQN, 0) && appendMember(method, alreadyMarkedToBeChecked, results))
			{
	...
	private static boolean appendMember(final PsiMember psiMember, final Set<PsiMember> underConsideration, final Map<PsiClass, Map<PsiMethod, List<String>>> results)
	{
	...
	public static void collectTestMethods(Map<PsiClass, Map<PsiMethod, List<String>>> classes, final PsiClass psiClass, final String methodName, final GlobalSearchScope searchScope)
	{
