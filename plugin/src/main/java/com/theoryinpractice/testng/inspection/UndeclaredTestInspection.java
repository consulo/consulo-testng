	...
	public ProblemDescriptor[] checkClass(@NotNull final PsiClass aClass, @NotNull final InspectionManager manager, final boolean isOnTheFly, Object state)
	{
		if(TestNGUtil.hasTest(aClass) && PsiClassUtil.isRunnableClass(aClass, true))
		{
			...
			return new ProblemDescriptor[]{
					manager.createProblemDescriptor(nameIdentifier, "Undeclared test \'" + aClass.getName() + "\'", isOnTheFly, new LocalQuickFix[]{
				...
				public void run()
				{
					...
					if(file != null)
					{
						...
						new WriteCommandAction(project, getName(), null)
						{
							...
							protected void run(final Result result) throws Throwable
							{
								XmlFile testngXml = (XmlFile) PsiFileFactory.getInstance(psiManager.getProject()).createFileFromText("testng.xml",
										XmlFileType.INSTANCE, "<!DOCTYPE suite SYSTEM \"http://testng.org/testng-1.0.dtd\">\n<suite></suite>");
								try
								{
