...
public class TestNGUtil
{
	...
	public static boolean hasTest(PsiModifierListOwner element, boolean checkHierarchy, boolean checkDisabled, boolean checkJavadoc)
	{
		...
		else
		{
			...
			if(psiClass != null)
			{
				final PsiAnnotation annotation = checkHierarchy ? AnnotationUtil.findAnnotationInHierarchy(psiClass, Collections.singleton(TEST_ANNOTATION_FQN)) : AnnotationUtil.findAnnotation
	...
	public static void collectAnnotationValues(final Map<String, Collection<String>> results, PsiMethod[] psiMethods, PsiClass... classes)
	{
		...
		if(psiMethods != null)
		{
			...
			{
				ApplicationManager.getApplication().runReadAction(() -> appendAnnotationAttributeValues(results, AnnotationUtil.findAnnotation(psiMethod, test), psiMethod));
			}
		}
		else
		{
	...
	private static void appendAnnotationAttributeValues(final Map<String, Collection<String>> results, final PsiAnnotation annotation, final PsiDocCommentOwner commentOwner)
	{
	...
	public static boolean checkTestNGInClasspath(PsiElement psiElement)
	{
		...
		if(JavaPsiFacade.getInstance(manager.getProject()).findClass(TestNG.class.getName(), psiElement.getResolveScope()) == null)
		{
			...
			{
				if(Messages.showOkCancelDialog(psiElement.getProject(), "TestNG will be added to module classpath", "Unable to Convert.", Messages.getWarningIcon()) != Messages.OK)
				{
					return false;
				}
			}
	...
	public static Version detectVersion(Project project, @Nullable Module module)
	{
		...
		return CachedValuesManager.getManager(project).getCachedValue(module, () ->
		{
			...
			PsiClass aClass = psiFacade.findClass("org.testng.internal.Version", GlobalSearchScope.moduleWithDependenciesAndLibrariesScope(module));
			if(aClass != null)
			{
